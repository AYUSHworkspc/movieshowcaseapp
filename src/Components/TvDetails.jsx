...
import Loding from './Loding';
  ...
  const {pathname}=useLocation(); //YAHA PATH NAME MILTA HAIN JO LOCATION DETA HAIN 
   ...
   const {info}= useSelector((state)=>state.tv)  //redux ka data access 
  ...
  <div className='content ml-[25%]'>
    <h1 className='text-5xl font-black text-white'>
      ...
      <span className='text-xl font-bold text-zinc-200'>{info.detail.first_air_date.split("-")[0]}</span>
    </h1>
  ...
  <div className='flex text-zinc-500 items-center gap-x-4 '>
    ...
    <h1  className='text-white'>{info.detail.first_air_date}</h1>
    <h1 className='text-yellow-200 font-bold'>   <span className='text-blue-300'>tv type=</span>  {info.detail.genres.map((e)=>e.name).join(",")}</h1>
    <h1 className='text-orange-500 font-bold'> <span> tv Duration</span>   {info.detail.runtime}</h1>
    </div>
  ...
  <i class="ri-tv-fill text-blue-500 mr-3 text-3xl "></i>
  Play Trailer</Link>
